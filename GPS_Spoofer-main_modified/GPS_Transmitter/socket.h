...
int raw_bits[MAX_CHAN+1];
double raw_bits_r[MAX_CHAN+1];
double llhr[3] = {7.724436,77.033146,100};
double dynamic_dt = 0;
double tow_correction = 0;
bool tow_rx;
bool tow_fix;
bool use_hash = false;
...
void bitstreamer_thread(void *in_struct)
{
    ...
    while (1)
    {
        // Receive (MAX_CHAN + 1) double values over UDP
        udprecv(s, raw_bits_r, (MAX_CHAN + 1) * sizeof(double));
        // Convert each double to int and store in raw_bits
        for (int nnm = 0; nnm < MAX_CHAN + 1; nnm++) {
             raw_bits[nnm] = (int)raw_bits_r[nnm];
            }
...
void tow_thread()
{
    int s_tow = udpinit(1575);
    tow_rx = false;
    tow_fix = false;
    
    while (1) {
        if (!tow_rx) {
            udprecv(s_tow, &tow_correction, sizeof(double));
            
            // Check if the received TOW correction meets the validity criteria.
            // For this example, we consider a valid correction as a positive number.
            if (tow_correction > 0) {
                printf("Valid TOW Correction %f\n", tow_correction);
                tow_fix = true;
                tow_rx = true;
                break;  // Exit the loop once valid data is received.
            } else {
                // Print a message if the data is invalid and continue waiting.
                printf("Invalid TOW Correction %f, waiting for valid data...\n", tow_correction);
            }
        }
    }
    
    // Optionally, close the socket if it's no longer needed.
   // close(s_tow);
}
...
void dt_thread()
{
    printf("Listening for dt corrections stream\n");
