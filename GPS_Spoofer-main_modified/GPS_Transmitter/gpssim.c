...
{   
    ...
    pthread_t th_tow;
    pthread_create(&th_tow, NULL, (void *)tow_thread, NULL);
    ...
    for (iumd = 1; iumd < numd; iumd++)
    {
        ...
        for (i = 0; i < MAX_CHAN; i++)
        {
            if (chan[i].prn > 0)
            {
                ...
                // Signal gain
                gain[i] = (int)(path_loss * ant_gain * 128.0 * 6); // scaled by 2^7
                //gain[i] = (int) (2000);
            }
        }
        ...
        // ---------------------------------------------------------
        // Added code: Print IQ values after generating the samples
        // ---------------------------------------------------------
        //printf("Generated I/Q Samples:\n");
        //for (int sample = 0; sample < iq_buff_size; sample++)
        //{
            // printf("Sample %d: I = %d, Q = %d\n", sample,
                     //iq_buff[sample * 2], iq_buff[sample * 2 + 1]);
        //}
        // ------------------------------------------------------
        ...
        if (igrx % 300 == 0) // Every 30 seconds
        {
            ...
            if (verb)
            {
                ...
                {
                    if (chan[i].prn > 0)
                        printf("%02d %6.1f %5.1f %11.1f %5.1f %d\n", chan[i].prn,
                               chan[i].azel[0] * R2D, chan[i].azel[1] * R2D, chan[i].rho0.d,
                               chan[i].rho0.iono_delay,gain[i]);
                }
            }
        }
